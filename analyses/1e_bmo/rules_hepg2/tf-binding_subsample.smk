...
for sample, depths in subsample_depths.items():
    rule:
        ...
        output:
            protected(expand(join(BAM_SUB_DIR, "{sample}.{depth}M.bam"), 
                     sample=sample, depth=depths)),
            expand(join(BAM_SUB_DIR, "{sample}.{depth}M.bam.bai"), 
                   sample=sample, depth=depths)
        resources:
            io_limit = 2
        run:
            for depth in depths:
                if depth < depths[-1]:
                    fraction = subsampled_bam_fraction_from_depth(sample, depth)
...
rule blacklist_filter:
    ...
    shell:
        """
        ionice -c2 -n7 intersectBed -v -a {input} \
            -b {params.BL1} {params.BL2} | awk '$9>1.30103{{print}}' \
            > {output}
        """
...
rule motifs_in_peaks:
    ...
    shell:
        ...
        ionice -c2 -n7 intersectBed -f 1.0 -loj -a {input.motif} \
            ...
            awk '{{OFS="\\t"; if($7 == ".") {{print $1,$2,$3,$4,$5,$6,0}} \
                else {{print $1,$2,$3,$4,$5,$6,1}}}}' \
            > {output}
        """
