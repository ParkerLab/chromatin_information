...
rule get_roc:
    ...
    output:
        ok = join(RESULTS, "roc", "{sample}", "get_roc.ok")
    params:
        join(RESULTS, "roc")
    run:
        with open(input[0], 'r') as associations:
            with open("sh.roc", 'w') as sh_file:
                for line in associations.readlines():
                    ...
                    # tp_file  = "/lab/work/albanus/gm12878/motif_intersects_encode2017/intersections_1000G/{}.bed".format(tp_name)
                    tp_file  = "/lab/work/albanus/2019_encode_chipseq_redo/intersections_1000G/gm12878/{}.bed".format(tp_name)
                    for sample, depths in subsample_depths.items():
                        for depth in depths:
                            infile = join(RESULTS, "concatenated_files", "{}/{}.{}.bed".format(sample,motif,depth))
                            outdir = "{}/{}/{}".format(params[0],sample,depth)
                            shell("mkdir -p {outdir}")
                            # print(infile)
                            if os.path.isfile(infile):
                                shell_cmd = "ionice -c2 -n7 Rscript ~/scripts/get_ROC_all.R {} 7 {} {} {} {}".format(
...
rule get_f1:
    input:
        ...
        bins = rules.make_bin_names_file.output
    output:
        ok = join(RESULTS, "f1/{sample}/get_f1.ok")
    params:
        join(RESULTS, "f1")
    run:
        with open(input[0], 'r') as associations:
            with open("sh.f1", 'w') as sh_file:
                for line in associations.readlines():
                    ...
                    for sample, depths in subsample_depths.items():
                        for depth in depths:
                            infile = join(RESULTS, "concatenated_files/{}/{}.{}.bed".format(sample,motif,depth))
                            peaks  = join(RESULTS, "motifs_in_atac-seq_peaks/{}/{}.{}.bed".format(sample,motif,depth))
                            piq = join(PIQPE_DIR, "processed_output_binary/{}/{}M.{}.bed".format(sample, depth, motif))
                            outdir = "{}/{}/{}".format(params[0],sample,depth)
                            shell("mkdir -p {outdir}")
                            if os.path.isfile(infile):
                                # shell_cmd = "ionice -c2 -n7 Rscript bin/calculate_F1.R -f {} -c 7 -n {} -t {} -b {} -p {} --piq {} -o {}".format(
                                #     infile, tp_name, tp_file, input[1], peaks, piq, outdir
                                # )
                                shell_cmd = "ionice -c2 -n7 Rscript bin/calculate_F1_noPIQ.R -f {} -c 7 -n {} -t {} -b {} -p {} -o {}".format(
...
rule aggregate_f1:
    input:
        rules.get_f1.output
    output:
        join(RESULTS, "f1/{sample}/f1_data.txt")
    run:
...
rule aggregate_roc:
    ...
    output:
        join(RESULTS, "roc/{sample}/roc_data.txt")
    run:
        with open(output[0],"w") as out:
...
rule aggregate_prauc:
    input:
        rules.get_roc.output
    output:
        join(RESULTS, "roc/{sample}/prauc_data.txt")
    run:
        with open(output[0],"w") as out:
            header = ["sample", "depth", "factor", "BMO", "CENTIPEDE", 
                      "DNase2TF", "HINT", 
