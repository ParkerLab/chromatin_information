...
#         piq = "tmp_{sample}.{motif}.{depth}.piq.tmp",
...
#         ionice -c2 -n7 cut -f 5 {input.piq} > {params.piq} &&
#         ionice -c2 -n7 zcat {input.centipede_sb} | cut -f 7 \
#             > {params.centipede_sb} &&
#         ionice -c2 -n7 paste {params.motif} {params.bmo} {params.centipede} \
#             {params.d2tf} {params.hint} {params.piq} {params.centipede_sb} \
#             > {output}  &&
#         rm {params.motif} {params.bmo} {params.centipede} {params.d2tf} \
#             {params.hint} {params.piq} {params.centipede_sb}
#         """

## Evaluate methods - no PIQ
rule concatenate_predictions:
    input:
        motif = os.path.join(MOTIF_DIR, "{motif}.bed.gz"),
        bmo = os.path.join(
                ...
                for line in associations.readlines():
                    ...
                    for sample, depths in subsample_depths.items():
                        for depth in depths:
                            ...
                            piq = os.path.join(PIQPE_DIR, "processed_output_binary/{}/{}M.{}.bed".format(sample, depth, motif))
                            outdir = "{}/{}/{}".format(params[0],sample,depth)
                            shell("mkdir -p {outdir}")
                            if os.path.isfile(infile):
