...
OLD_DIR = "/lab/work/albanus/2018_redo_all/work/tf-binding/DNase2TF/{sample}"
...
rule DNase2TF:
    input:
        dft = os.path.join(
            OLD_DIR, "input/dinuc_freq_table_ac_{sample}.{depth}M.txt"
        ),
        peaks = rules.prepare_peaks.output,
        indir = os.path.join(OLD_DIR, "input/{sample}.{depth}_offset/done.ok")
    output:
        protected(
            os.path.join(
                D2TF_DIR,
    ...
    params:
        script = os.path.join(
            "/home/albanus/analyses/2017",
            "buenrostro_individual_samples/scripts/run.R"
        ),
        bam_handle = os.path.join(
            OLD_DIR, "input/{sample}.{depth}_offset/{sample}.{depth}"
        ),
