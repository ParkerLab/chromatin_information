...
c1 <- d_frap %>% 
  ...
  summarise(residual_corr = cor.test(residual, log10(frap_time), method = 's', exact = F)$estimate,
            ...
            method = "spearman") %>% 
  mutate(adj_p = p.adjust(residual_p, method = "holm")) %>% 
  mutate(adj_p_fvice = p.adjust(fvice_p, method = "holm"))
...
c2 <- d_frap %>% 
  ...
  summarise(residual_corr = cor.test(residual, log10(frap_time), method = 'p', exact = F)$estimate,
            ...
            method = "pearson") %>% 
  mutate(adj_p = p.adjust(residual_p, method = "bonferroni")) %>% 
  mutate(adj_p_fvice = p.adjust(fvice_p, method = "bonferroni"))
...
c3 <- d_frap %>% 
  ...
  mutate(adj_p_rr = p.adjust(residual_p_rr, method = "holm"))
...
p <- corrs %>% 
  mutate(sample = factor(sample, levels = samples, labels = labs, ordered = T)) %>% 
  ggplot(aes(x = sample, y = residual_corr, fill = adj_p <= 0.05)) +
  geom_col() +
  facet_wrap(~ method) +
